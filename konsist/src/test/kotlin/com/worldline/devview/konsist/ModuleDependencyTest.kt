    ...
    FunSpec(body = {
        test(name = "DevView feature modules depend on DevView core module") {
            ...
            featureModules.forEach { moduleName ->
                ...
                withClue(
                    "'$moduleName' has no file importing from the core '$BASE_PACKAGE' package"
                ) {
        ...
        test(name = "Each DevView feature family has exactly one -core sub-module") {
            ...
            val subModules = allModules.filter {
                it
                    .removePrefix(
                        DEVVIEW_MODULE_PREFIX
                    ).contains("-")
            }
        ...
        test(name = "DevView feature modules depend on their -core sub-module") {
            val allModules = devviewFeatureModuleNames()
            val featureModules = allModules.filter {
                !it
                    .removePrefix(
                        DEVVIEW_MODULE_PREFIX
                    ).contains("-")
            ...
            featureModules.forEach { moduleName ->
                ...
                val hasImportFromCoreModule = Konsist
                    .scopeFromModule(moduleName)
                    .files
                    .any { file ->
                        file.imports.any { import ->
                ...
                withClue(
                    "'$moduleName' has no file importing from core sub-module package '$corePackage'"
                ) {
        ...
        test(name = "DevView utils module does not depend on any DevView module") {
            val hasImportFromDevView = Konsist
                .scopeFromModule(DEVVIEW_UTILS_MODULE)
                .files
                .any { file ->
                    file.imports.any { import ->
                        import.name.startsWith(BASE_PACKAGE) &&
