...
- **Sort control**: a `MoreVert` overflow menu, top-right of the screen's own content (above the global toggle) — sort keys are spec order (default, document order), path (A-Z), method (`GET`/`POST`/`PUT`/`PATCH`/`DELETE`/`HEAD`/`OPTIONS` order), and tag (an operation's first declared tag, untagged operations first); "Tag" is hidden from the menu entirely when the current tab has no tagged operations. Selection is per-tab, like the version and method filters, and — like every filter here — pure client-side state, not persisted or round-tripped through the ViewModel.
...
- **Tag filter**: a per-tab row of chips, one per distinct `Operation.tag` present among that tab's operations (see [Tags](networkmock-core.md#tags)), alphabetically ordered. Multi-select, same semantics as the method filter. Hidden entirely when a spec has no tagged operations.
